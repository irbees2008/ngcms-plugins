...
class AutoKeysNewsFilter extends NewsFilter
{
	...
	{
		if (!empty($_POST['autokeys_generate']) && $_POST['autokeys_generate'] == 1) {
			$SQL['keywords'] = akeysGetKeys([
				'content' => $SQL['content'] ?? '',
				'title' => $SQL['title'] ?? ''
			]);
		}
	...
	public function editNewsForm($newsID, $SQLold, &$tvars)
	{
		...
		$tvars['plugin']['autokeys'] = $xt->render([
			'flags' => ['checked' => pluginGetVariable('autokeys', 'activate_edit')]
		]);
	...
	public function addNewsForm(&$tvars)
	{
		...
		$tvars['plugin']['autokeys'] = $xt->render([
			'flags' => ['checked' => pluginGetVariable('autokeys', 'activate_add')]
		]);
