...
function cat_edit()
{
	...
	if (!empty($error_text)) {
		foreach ($error_text as $error) {
			...
			notify('error', $error);
			logger("zboard config cat edit error: {$error}", 'warning');
		}
	} else {
		$error_input = '';
	}
...
function getCats()
{
	...
	$tree = array();
	$cur = array();
	$categories = $mysql->select("SELECT * FROM " . prefix . "_zboard_cat ORDER BY id");
...
function send_cat($params)
{
	...
	if (!empty($error_text)) {
		foreach ($error_text as $error) {
			$error_input .= '<div class="alert alert-danger">' . $error . '</div>';
			notify('error', $error);
			logger("zboard config cat add error: {$error}", 'warning');
		}
	} else {
		$error_input = '';
	}
...
function list_price()
{
	...
	foreach ($mysql->select('SELECT * from ' . prefix . '_zboard_pay_price ORDER BY id ASC') as $row) {
		$gvars['vars'] = array(
			...
			'price_del' => '<a href="?mod=extra-config&plugin=zboard&action=price_del&id=' . $row['id'] . '"  /><i class="fa fa-trash-o" aria-hidden="true"></i></a>',
		);
...
function send_price($params)
{
	...
	if (!empty($error_text)) {
		foreach ($error_text as $error) {
			$error_input .= '<div class="alert alert-danger">' . $error . '</div>';
			notify('error', $error);
			logger("zboard config price add error: {$error}", 'warning');
		}
	} else {
		$error_input = '';
	}
...
function price_edit()
{
	...
	if (!empty($error_text)) {
		foreach ($error_text as $error) {
			$error_input .= '<div class="alert alert-danger">' . $error . '</div>';
			notify('error', $error);
			logger("zboard config price edit error: {$error}", 'warning');
		}
	} else {
		$error_input = '';
	}
		...
		*/
		...
		if (!empty($error_text)) {
			foreach ($error_text as $error) {
				$error_input .= '<div class="alert alert-danger">' . $error . '</div>';
				notify('error', $error);
				logger("zboard config announce error: {$error}", 'warning');
			}
		} else {
			$error_input = '';
		}
...
function modify()
{
	...
	logger('zboard announce deleted: ' . $result, 'info');
}
function main()
{
	...
	if (isset($_REQUEST['submit'])) {
		...
		// Robokassa settings
		pluginSetVariable('zboard', 'robokassa_login', $_REQUEST['robokassa_login']);
		pluginSetVariable('zboard', 'robokassa_pass1', $_REQUEST['robokassa_pass1']);
		pluginSetVariable('zboard', 'robokassa_pass2', $_REQUEST['robokassa_pass2']);
		pluginSetVariable('zboard', 'robokassa_is_test', intval($_REQUEST['robokassa_is_test']));
		pluginsSaveConfig();
...
*/
	$pvars['vars'] = array(
		...
		'robokassa_pass1' => $robokassa_pass1,
		'robokassa_pass2' => $robokassa_pass2,
