...
function plugin_jchat_show()
{
    ...
    logger('Show: lastEvent=' . $lastEvent . ', start=' . $start, 'info', 'jchat.log');
...
function plugin_jchat_add()
{
    ...
    if (is_array($userROW)) {
        ...
        if (!trim($name)) {
            logger('Add failed: No name specified, IP=' . $ip, 'info', 'jchat.log');
    ...
    if (!trim($text)) {
        logger('Add failed: No text specified, author=' . ($SQL['author'] ?? 'unknown'), 'info', 'jchat.log');
    ...
    if (!is_array($userROW) && (pluginGetVariable('jchat', 'access') < 2)) {
        logger('Add failed: Guest not allowed, IP=' . $ip, 'info', 'jchat.log');
    ...
    if (is_array($mysql->record("select id from " . prefix . "_jchat where (ip = " . db_squote($ip) . ") and (postdate + " . $rate_limit . ') > ' . time()))) {
        logger('Add failed: Rate limit exceeded, IP=' . $ip, 'info', 'jchat.log');
    ...
    logger('Message added: author=' . $SQL['author'] . ', author_id=' . ($SQL['author_id'] ?? 0) . ', IP=' . $ip, 'info', 'jchat.log');
...
function plugin_jchat_del()
{
    ...
    if (!is_array($userROW) || ($userROW['status'] > 1)) {
        logger('Delete failed: Permission denied, IP=' . $ip, 'info', 'jchat.log');
    ...
    if (!($crow = $mysql->record("select * from " . prefix . "_jchat where id = " . db_squote($id)))) {
        logger('Delete failed: Item not found, id=' . $id . ', admin=' . ($userROW['name'] ?? 'unknown'), 'info', 'jchat.log');
    ...
    logger('Message deleted: id=' . $id . ', admin=' . ($userROW['name'] ?? 'unknown'), 'info', 'jchat.log');
    // Update LastEventNotification
