...
array_push($cfg, array('name' => 'secure', 'title' => $lang['voting:secure'], 'descr' => $lang['voting:secure#desc'], 'type' => 'select', 'values' => array('1' => $lang['voting:secure.db'], '0' => $lang['voting:secure.cookie']), 'value' => pluginGetVariable('voting', 'secure')));
...
if ($_REQUEST['action'] == 'newvote') {
	$mysql->query("insert into " . prefix . "_vote (name) values (" . db_squote($lang['voting:new_vote_default']) . ")");
	print $lang['voting:msg.vote_created'] . " <a href='$PHP_SELF?mod=extra-config&plugin=voting'>" . $lang['voting:msg.redirect'] . "</a>";
	...
	if ($row = $mysql->record("select * from " . prefix . "_vote where id = $voteid")) {
		...
		print $lang['voting:msg.vote_deleted'] . " ";
	} else {
		print $lang['voting:msg.vote_notfound'] . " ";
	}
	print "<a href='$PHP_SELF?mod=extra-config&plugin=voting'>" . $lang['voting:msg.redirect'] . "</a>";
