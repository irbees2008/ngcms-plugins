...
**Версия плагина:** v1.3
**PHP совместимость:** 8.0+

---

## v1.3 (2026-02-03) - Модернизация до ng-helpers v0.2.2

### Изменения

- ✅ Обновлены импорты до ng-helpers v0.2.2
- ✅ Заменена функция `cacheRetrieveFile()` на универсальную `cache()`
- ✅ Заменена функция `cache_put()` на универсальную `cache()`
- ✅ Улучшено логирование с детальными метриками
- ✅ Кеш-ключи изменены с `sitemap_X.txt` на `sitemap_page_X`
- ✅ Добавлено логирование попаданий в кеш с уровнем debug
- ✅ Добавлен размер результата в логи

### Технические детали

- **cache()**: Универсальная функция кеширования (get/set в одном вызове)
- **logger()**: Улучшенное логирование в `engine/data/logs/sitemap.log`
- **Логи**: info - сохранение, debug - попадания в кеш
- **Требования**: ng-helpers v0.2.2+, PHP 8.0+

### Пример использования cache() v0.2.2

```php
// Чтение из кеша (с callback, возвращающим null)
$cached = cache($cacheKey, function() { return null; }, $cacheExpire);
if ($cached !== null) {
    // Данные из кеша
}

// Запись в кеш (callback возвращает данные)
cache($cacheKey, function() use ($result) { return $result; }, $cacheExpire);
```

---

## v1.2 (2026-01-11) - ng-helpers v0.2.0
