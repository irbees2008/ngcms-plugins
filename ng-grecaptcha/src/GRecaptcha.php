...
class GRecaptcha
{
    ...
    public function configure(array $params = []): self
    {
        ...
        $this->userToken = trim(sanitize($_POST['g-recaptcha-response'] ?? ''));
    ...
    public function verifying()
    {
        try {
            ...
            logger('ng-grecaptcha: Verification successful, score: ' . $verified->score, 'info');
            return true;
        } catch (MissingVariableException $e) {
            $this->rejectionReason = $e->getMessage();
            logger('ng-grecaptcha: Missing variable - ' . $e->getMessage(), 'warning');

            return false;
        } catch (VerificationFailedException $e) {
            $this->rejectionReason = $e->getMessage();
            logger('ng-grecaptcha: Verification failed - ' . $e->getMessage(), 'warning');

            return false;
        } catch (Throwable $e) {
            logger('ng-grecaptcha: Unexpected error - ' . $e->getMessage(), 'error');
    ...
    protected function touchAnswer(): stdClass
    {
        ...
        if (extension_loaded('curl') and function_exists('curl_init')) {
            ...
            logger('ng-grecaptcha: Neither cURL nor allow_url_fopen available', 'error');
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {
            logger('ng-grecaptcha: JSON decode error - ' . json_last_error_msg(), 'error');
    ...
    protected function getCurlAnswer(string $query)
    {
        $ch = curl_init();
        if (curl_errno($ch) != 0) {
            $error = 'err_curl_' . curl_errno($ch) . ' ' . curl_error($ch);
            logger('ng-grecaptcha: cURL init error - ' . $error, 'error');
        ...
        if (404 === $status) {
            logger('ng-grecaptcha: API endpoint not found (404)', 'error');
            throw new RuntimeException('Source file not found.');
        } elseif ($status !== 200) {
            $error = 'err_curl_' . $status;
            logger('ng-grecaptcha: API HTTP error - ' . $error, 'error');
            throw new RuntimeException($error);
        }
