        ...
        foreach ($blockRecList as $blockIndexNum) {
            ...
            if ($blockInfo['type'] != 1) {
                ...
                if (is_array($row = $mysql->record('select ads_blok from ' . prefix . '_ads_pro where id=' . db_squote($blockIndexNum)))) {
                    // Get content based on type: 0=HTML, 2=TEXT
                    if ($blockInfo['type'] == 0) {
                        // HTML type - output as is (admin is responsible for content)
                ...
                // Execute PHP block and capture output
                $out2 = '';
                if ($description) {
                    ob_start();
                    try {
                        ...
                        logger('PHP block error: id=' . $blockIndexNum . ', error=' . $e->getMessage(), 'error', 'ads_pro.log');
                    }
                    ob_end_clean();
                }

                logger('PHP block executed: id=' . $blockIndexNum . ', output=' . strlen($out2) . ' bytes', 'info', 'ads_pro.log');
