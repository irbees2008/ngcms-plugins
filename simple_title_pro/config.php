...
function about()
{
	...
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.about'],
...
function clear_cache()
{
	...
	$_SESSION['simple_title_pro']['notify'] = $userROW['name'] . ', ' . $lang['simple_title_pro#notify.cache_cleared'];
...
function list_news()
{
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_news as n ON (n.id = s.news_id) WHERE s.news_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'   => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_news&id=' . $row['sid'] . '"  />' . $lang['simple_title_pro#btn.delete'] . '</a>',
		);
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.list_news'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function license()
{
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.about'] . ' => ' . $lang['simple_title_pro#page.license'],
...
function list_static()
{
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_static as n ON (n.id = s.static_id) WHERE s.static_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'   => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_static&id=' . $row['sid'] . '"  />' . $lang['simple_title_pro#btn.delete'] . '</a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.list_static'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function list_cat()
{
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, c.name as cname FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_category as c ON (c.id = s.cat_id) WHERE s.cat_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'      => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_cat&id=' . $row['sid'] . '"  />' . $lang['simple_title_pro#btn.delete'] . '</a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.list_cat'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function send_title()
{
	...
	switch ($_REQUEST['do']) {
		case 'cat':
			...
			if (isset($_REQUEST['submit'])) {
				...
				if (empty($title)) {
				$error_text[] = $lang['simple_title_pro#err.title_required'];
			}
			if (empty($id)) {
				$error_text[] = $lang['simple_title_pro#err.id_missing'];
			}
			...
			if ($exists === false) {
				$error_text[] = $lang['simple_title_pro#err.db_check'];
			} elseif ($exists > 0 && empty($edit)) {
				$error_text[] = $lang['simple_title_pro#err.cat_exists'];
		...
		case 'news':
			...
			if ($exists === false) {
				...
				$error_text[] = $lang['simple_title_pro#err.news_exists'];
				if (empty($error_text)) {
		...
		case 'static':
			...
			if ($exists === false) {
				...
				$error_text[] = $lang['simple_title_pro#err.static_exists'];
				if (empty($error_text)) {
					if (empty($_REQUEST['edit'])) {
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.send_title'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function list_all()
{
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_static as n ON (n.id = s.static_id) WHERE s.static_id != 0 ORDER BY s.id ASC') as $row) {
		...
		$tVars = array(
			...
			'cat_name_del' => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del&id=' . $row['id'] . '" />' . $lang['simple_title_pro#btn.delete'] . '</a>',
		);
		$entries .= $xe->render($tVars);
	}
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_news as n ON (n.id = s.news_id) WHERE s.news_id != 0 ORDER BY s.id ASC') as $row) {
		...
		$tVars = array(
			...
			'cat_name_del' => '<a href="?mod=extra-config&plugin=simple_title_pro&action=cat_name_del&id=' . $row['id'] . '"  />' . $lang['simple_title_pro#btn.delete'] . '</a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => $lang['simple_title_pro#page.list_all'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function main()
{
	...
	$tVars = array(
		'c_title'      => array(
			'print' => $c_title,
			'error' => empty($c_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home%  / %cat% %num%' : ''
		),
		'n_title'      => array(
			'print' => $n_title,
			'error' => empty($n_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home%  / %cat% / %title%  %num%' : ''
		),
		'm_title'      => array(
			'print' => $m_title,
			'error' => empty($m_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home% %num%' : ''
		),
		'static_title' => array(
			'print' => $static_title,
			'error' => empty($static_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home% / %static%' : ''
		),
		'o_title'      => array(
			'print' => $o_title,
			'error' => empty($o_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home% / %other% %html% %num%' : ''
		),
		'e_title'      => array(
			'print' => $e_title,
			'error' => empty($e_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '%home% / %other%' : ''
		),
		'num_title'    => array(
			'print' => $num_title,
			'error' => empty($num_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '/ Страница %count%' : ''
		),
		'num_cat'      => array(
			'print' => $num_cat,
			'error' => empty($num_cat) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '20' : ''
		),
		'num_news'     => array(
			'print' => $num_news,
			'error' => empty($num_news) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '20' : ''
		),
		'num_static'   => array(
			'print' => $num_static,
			'error' => empty($num_static) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '20' : ''
		),
		'html_secure'  => array(
			'print' => $html_secure,
			'error' => empty($html_secure) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '/ %html%' : ''
		),
		'p_title'      => array(
			'print' => $p_title,
			'error' => ''
		...
		'cache'        => array(
			'print' => $cache,
			'error' => empty($cache) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />' . $lang['simple_title_pro#err.field_empty'] . '1' : ''
		),
	);
	foreach ($tVars as $row) {
		//print "<pre>".var_export($row['error'], true)."</pre>";
		if (!empty($row['error'])) {
			$info .= '<div class="alert alert-warning">' . $lang['simple_title_pro#err.has_errors'] . '</div>';
			break;
		}
	}
	$tVars = array(
		...
		'global'  => $lang['simple_title_pro#page.main'],
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
function del_cat()
{
	...
	$id = intval($_REQUEST['id']);
	if (empty($id)) {
		$_SESSION['simple_title_pro']['notify_error'] = $lang['simple_title_pro#err.no_selection'];
		return;
	}
	$mysql->query("delete from " . prefix . "_simple_title_pro where id = {$id} LIMIT 1");
	$_SESSION['simple_title_pro']['notify'] = $lang['simple_title_pro#notify.record_deleted'];
}
function del_news()
{
	global $mysql, $lang;
